        ...
        <table className={className.join(' ')}>
            <tbody>
                {grid.map((rowCells, rowIndex) => {
                    return (
                        <tr>
                            {rowCells.map((cell: GridCell | Element, cellIndex: number) => {
                                if (cell === EMPTY_CELL) return <td key={cellIndex}></td>;

                                const { col, row, colSpan, dataKey, render, rowSpan } =
                                    cell as GridCell;
